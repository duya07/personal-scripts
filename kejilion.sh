	...
	case $sub_choice in
	  ...
	  81|jitsi)
		  ...
		  docker_app_install() {
			  ...
			  wget $(wget -q -O - https://v6.gh-proxy.org/https://api.github.com/repos/jitsi/docker-jitsi-meet/releases/latest | grep zip | cut -d\" -f4)
			  unzip "$(ls -t | head -n 1)"
			  cd "$(ls -dt */ | head -n 1)"
			  cp env.example .env
			  ./gen-passwords.sh
...
while true; do
	...
	case "$choice" in
		...
		2)
			...
			if [ "$country" = "CN" ]; then
				cron_proxy="https://v6.gh-proxy.org/"
