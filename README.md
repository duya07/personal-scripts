...
修改说明：
- 仅关闭默认上传功能，同步至上游 v4.5.10。
- 上游自有域名与 GitHub API 调用改走 v6.gh-proxy.org 代理。
- 脚本内置的「自动更新」改为从本仓库拉取，不会覆盖以上改动。

安装并运行：

```bash
sudo wget -O kejilion.sh https://v6.gh-proxy.org/https://raw.githubusercontent.com/duya07/personal-scripts/main/kejilion.sh && chmod +x kejilion.sh && ./kejilion.sh
```
